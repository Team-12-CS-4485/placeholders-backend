...
def get_latest_videos(
    youtube, uploads_playlist_id: str, max_results=MAX_VIDEOS_PER_CHANNEL
):
    # Since we have so many filter lets fetch more videos than we need and then filter down to the top 5
    # that meet all criteria. This way we have a better chance of getting 5 good videos per channel.
...
def save_to_s3(channel_name, videos):
    ...
    try:
        ...
        s3_key = f"youtube-data/week5/{channel_name.lower()}.json"
    ...
    try:
        for video in unique_videos:
            ...
            try:
                ...
                table.put_item(
                    Item=item, ConditionExpression="attribute_not_exists(PartitionKey)"
                )
                written += 1
            except ClientError as e:
                if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                    logger.info(
                        f"Video {video['videoId']} already exists in DynamoDB, skipping"
                    )
                    skipped += 1
                else:
                    raise
        ...
        logger.info(
            f"DynamoDB for {channel_name}: {written} new, {skipped} already existed"
        )
    except Exception as e:
    ...
    for idx, (channel_name, channel_id) in enumerate(NEWS_CHANNELS.items(), 1):
        try:
            ...
            videos = get_latest_videos(
                youtube, uploads_playlist, MAX_VIDEOS_PER_CHANNEL
            )
            ...
            if not videos:
                ...
                results.append(
                    {"channel": channel_name, "status": "success", "videoCount": 0}
                )
                continue
            ...
            for video in videos:
                if len(filtered_videos) >= MAX_VIDEOS_PER_CHANNEL:
                    logger.info(
                        f"[{channel_name}] Reached target of {MAX_VIDEOS_PER_CHANNEL} videos, moving to next channel"
                    )
                    break
                ...
                try:
                    ...
                    if published_date < cutoff_date:
                        ...
                        logger.info(
                            f"[{channel_name}] Skipping {vid} - published {days_ago} days ago"
                        )
                        continue
                except Exception as e:
                    logger.warning(
                        f"[{channel_name}] Could not parse date for {vid}: {e}"
                    )
                    continue
                ...
                if view_count < MIN_VIEW_COUNT:
                    logger.info(
                        f"[{channel_name}] Skipping {vid} - only {view_count} views (min: {MIN_VIEW_COUNT})"
                    )
                    continue
                ...
                if not is_within_duration_limit(duration):
                    logger.info(
                        f"[{channel_name}] Skipping {vid} - exceeds {MAX_VIDEO_DURATION_MINUTES} min duration limit"
                    )
                    continue
                ...
                if len(top_comments) < COMMENTS_PER_VIDEO:
                    logger.info(
                        f"[{channel_name}] Skipping {vid} - only {len(top_comments)} comments (need {COMMENTS_PER_VIDEO})"
                    )
                    continue
                ...
                if not transcript:
                    logger.info(
                        f"[{channel_name}] Skipping {vid} - no transcript available"
                    )
                    continue
            ...
            logger.info(
                f"[{channel_name}] Filtered to {len(filtered_videos)}/{MAX_VIDEOS_PER_CHANNEL} videos after all checks"
            )
                ...
                results.append(
                    {
                        "channel": channel_name,
                        "status": "error",
                        "error": "No videos met filtering criteria",
                    }
                )
                continue
            ...
            try:
                save_to_dynamodb(
                    dynamodb, DYNAMODB_TABLE, channel_name, filtered_videos
                )
            except Exception as e:
                ...
                results.append(
                    {
                        "channel": channel_name,
                        "status": "error",
                        "error": f"DynamoDB save failed: {e}",
                    }
                )
                continue
            ...
            logger.info(
                f"[{channel_name}] Successfully processed {len(filtered_videos)} videos"
            )
            results.append(
                {
                    "channel": channel_name,
            ...
            results.append(
                {"channel": channel_name, "status": "error", "error": str(e)}
            )
